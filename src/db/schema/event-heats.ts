...
/**
 * Heats for an individual event. Keyed by `event_slug` text (no FK — events live
 * in the config registry, not the DB). `capacity` is capped at the event's
 * `bibPool` by the callers that create/update heats (see ADR 0003).
 *
